...
void GPUInstance::implement_cuda_function(){
  ...
  if(FUNC_COMP(function_name, cuDeviceGet)){
      ...
      using param_t = FunctionTraits<decltype(cuDeviceGetAttribute)>::ParameterTuple;
      param_t args; deserializer_ >> args;
      int pi;
      std::get<0>(args) = &pi;
      ...
      using param_t = FunctionTraits<decltype(cuDriverGetVersion)>::ParameterTuple;
      param_t args; deserializer_ >> args;
      int version;
      ...
      std::cout << "<<<<<<<<<<implemented as cuDevicePrimaryCtxRetain" << std::endl;
      using param_t = FunctionTraits<decltype(cuDevicePrimaryCtxRetain)>::ParameterTuple;
      param_t args; deserializer_ >> args;
      CUcontext ctx;
      std::get<0>(args) = &ctx;
      ...
      std::cout << "<<<<<<<<<<implemented as cuCtxGetStreamPriorityRange" << std::endl;
      using param_t = FunctionTraits<decltype(cuCtxGetStreamPriorityRange)>::ParameterTuple;
      param_t args; deserializer_ >> args;
      int least, greatest;
      ...
      using param_t = FunctionTraits<decltype(cuModuleGetLoadingMode)>::ParameterTuple;
      param_t args; deserializer_ >> args;
      ...
      using param_t = FunctionTraits<decltype(cuLibraryGetModule)>::ParameterTuple;
      param_t args; deserializer_ >> args;
      CUmodule module;
      ...
      response_.set_curesult(result);
      response_ << args;
  } else if(FUNC_COMP(function_name, cuOccupancyMaxActiveBlocksPerMultiprocessorWithFlags)){
      std::cout << "<<<<<<<<<<implemented as cuOccupancyMaxActiveBlocksPerMultiprocessorWithFlags" << std::endl;
      using param_t = FunctionTraits<decltype(cuOccupancyMaxActiveBlocksPerMultiprocessorWithFlags)>::ParameterTuple;
      param_t args; deserializer_ >> args;
      int numBlocks;
      std::get<0>(args) = &numBlocks;
      CUresult result = std::apply(cuOccupancyMaxActiveBlocksPerMultiprocessorWithFlags, args);
      response_.set_curesult(result);
      response_ << args;
      ...
      if(is_pinned(client_ptr)) std::get<1>(args) = convert_pin(client_ptr);
      else std::get<1>(args) = vgpu_ptr_;
      ...
      if(is_pinned(client_ptr)) std::get<0>(args) = convert_pin(client_ptr);
      else std::get<0>(args) = vgpu_ptr_;
  ...
  //     response_.set_curesult(result);
      ...
      param_t args; deserializer_ >> args;
      if(!client_pin_start_) client_pin_start_ = (void*)std::get<0>(args);
      registered_.push_back(pinned_memory_.register_pinned_memory(std::get<1>(args)));
      size_t alignment = 4096;  // page-aligned

      CUresult result = cuMemHostRegister(
          registered_.back(), std::get<1>(args), std::get<2>(args));
      std::cout << "Registering ptr: " << registered_.back() << " size: " << std::get<1>(args) << std::endl;
      ...
      CUuuid uuid_; deserializer_ >> uuid_;
      const void* ptable;
      ...
      using param_t = FunctionTraits<decltype(cuMemHostGetDevicePointer)>::ParameterTuple;
      param_t args; deserializer_ >> args;
      device_ptrs_.push_back(std::make_unique<CUdeviceptr>());
      std::get<0>(args) = device_ptrs_.back().get();
